...
from reportlab.platypus import SimpleDocTemplate, Paragraph, Spacer, Table, TableStyle, PageBreak  # Import PageBreak
...
def processar_ausencias(df):
    ...
    def converter_para_horas(tempo):
        ...
        try:
            ...
            return 0  # Return 0 if it's not a valid time format.
        except:
            return 0
...
def main():
    ...
    if uploaded_func is not None and uploaded_ausencias is not None and data_limite is not None:
        try:
            ...
            # Criar conteúdo HTML para o relatório.  Usando <div> e estilos inline para maior controle.
            html_content = """
            <div style="font-family: Arial; padding: 20px;">
                <h1 style="color: #1f77b4; text-align: center;">RELATÓRIO DE PRÊMIOS - VISÃO EXECUTIVA</h1>
                <p style="text-align: right; font-size: 14px;">Data do relatório: {}</p>
                <div style="display: flex; justify-content: space-between; margin-bottom: 20px;">
                    <div style="width: 23%; text-align: center; padding: 10px; background-color: #f8f9fa; border-radius: 5px;">
            ...
            for status in sorted(df_mostrar['Status'].unique()):
                ...
                <div style="margin-bottom: 20px; padding: 15px; background-color: #f8f9fa; border-radius: 5px;">
                    ...
                    <p><strong>Quantidade de Funcionários:</strong> {len(df_status):,.0f}</p>
            ...
            html_content += "</div>"  # Fecha a div principal
            ...
            if st.button("📑 Exportar Relatório como PDF"):
                try:
                    ...
                    resumo_data = [
                        ['Total Analisados', 'Com Direito', 'Aguardando Decisão', 'Valor Total'],
                        [f'{len(df_mostrar):,}', f'{len(df_mostrar[df_mostrar["Status"] == "Tem direito"]):,}',
                         f'{len(df_mostrar[df_mostrar["Status"].str.contains("Aguardando decisão", na=False)]):,}',
                         f'R$ {df_mostrar["Valor_Premio"].sum():,.2f}']
                    ]

                    t = Table(resumo_data, colWidths=[120, 120, 120, 120])  # Ajuste de larguras
                    t.setStyle(TableStyle([
                        ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor('#2c3e50')),  # Cabeçalho
                        ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),
                        ('ALIGN', (0, 0), (-1, -1), 'CENTER'),
                        ('FONTNAME', (0, 0), (-1, 0), 'Helvetica-Bold'),
                        ('FONTSIZE', (0, 0), (-1, 0), 10),
                        ('BOTTOMPADDING', (0, 0), (-1, 0), 12),
                        ('BACKGROUND', (0, 1), (-1, -1), colors.HexColor('#f8f9fa')),  # Corpo da tabela
                    ...
                    # Detalhamento por status (loop, como antes, mas agora com tabelas formatadas)
                    for status in sorted(df_mostrar['Status'].unique()):
                        df_status = df_mostrar[df_mostrar['Status'] == status]

                        story.append(Paragraph(f'Status: {status}', styles['SectionHeader']))

                        # Tabela de resumo do status
                        info_data = [
                            [f'Quantidade: {len(df_status):,}', f'Valor Total: R$ {df_status["Valor_Premio"].sum():,.2f}'],
                            [f'Locais: {", ".join(sorted(df_status["Local"].unique()))}', '']
                        ]
                        t_info = Table(info_data, colWidths=[300, 180]) #Larguras ajustadas
                        t_info.setStyle(TableStyle([
                            ...
                            ('BOTTOMPADDING', (0,0), (-1,0), 6) #padding na primeira linha
                        ...
                        # Tabela de detalhes do status
                        if len(df_status) > 0:
                            data = [['Matr.', 'Nome', 'Cargo', 'Local', 'Valor']]
                            for _, row in df_status.iterrows():
                                data.append([
                            ...
                            t = Table(data, colWidths=[50, 140, 100, 100, 70]) #ajuste de colunas
                            t.setStyle(TableStyle([
                                ...
                                ('TEXTCOLOR', (0, 1), (-1, -1), colors.black),
                                ('ALIGN', (4, 0), (4, -1), 'RIGHT'), # Alinhar valor à direita
                                ('FONTNAME', (0, 1), (-1, -1), 'Helvetica'),
                                ('FONTSIZE', (0, 1), (-1, -1), 8),
                                ('GRID', (0, 0), (-1, -1), 1, colors.grey),
                            ]))
            ...
            # ... (resto do código para Excel, etc., permanece o mesmo) ...
            # Estatísticas
            ...
            if st.button("Exportar Resultados"):
                ...
                # Corrigir o problema de KeyError, garantindo que 'Matricula' existe em df_funcionarios.
                if 'Matricula' in df_funcionarios.columns:
                     df_export['Salario'] = df_funcionarios.set_index('Matricula').loc[df_export['Matricula'], 'Salario_Mes_Atual'].values
                else:
                   st.error("A coluna 'Matricula' não foi encontrada na base de funcionários.") #Mensagem de erro
                   df_export['Salario'] = 0 #valor default
